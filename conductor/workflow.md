...
1. **Issue Tracking (Pre-work):** Before starting a new task, ensure it is tracked in **bd (beads)**.
   - Run `bd ready` to see unblocked work.
   - If the task doesn't exist, create it: `bd create "Task Description" --type task --priority 2`.
   - Note the issue ID.

2. **Select Task:** Choose the next available task from `plan.md` in sequential order.

3. **Mark In Progress:**
   - Edit `plan.md` and change the task from `[ ]` to `[~]`.
   - Update the **bd** issue status: `bd update <id> --status in_progress`.
...
7. **Verify Coverage:** Run coverage reports using the project's chosen tools (e.g., `bun test --coverage`).
...
11. **Close Issue:**
    - Mark the task as complete in **bd**: `bd close <id>`.
...
14. **Sync (Session End):** At the end of every work session, run `bd sync` followed by `git push`.
