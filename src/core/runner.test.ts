...
describe('Runner', () => {
  ...
  it('should treat hyphen state ids as the colon form when stored kind matches', async () => {
    const app = new App();
    const stack = new Stack(app, 'test');
    ...
    const TestRunnerLayer = RunnerLive.pipe(
      Layer.provide(MockState(
        { 'file-/tmp/x': { hash: 'hash', kind: 'file', metadata: { path: '/tmp/x' } } },
        (state) => { saved = state; },
      )),
    );
  ...
  it('should destroy a hyphen id whose stored kind does not match the prefix', async () => {
    const app = new App();
    const stack = new Stack(app, 'test');
    const res = new TestResource(stack, 'file:/tmp/x');
    let saved: AppState | undefined;
    ...
    expect(res.applied).toBe(true);
    expect(TestResource.destroyedIds).toEqual(['file-/tmp/x']);
    expect(saved?.['file:/tmp/x']).toBeDefined();
    expect(saved?.['file-/tmp/x']).toBeUndefined();
  });

  it('should not destroy a hyphen leftover when the colon key remains in the graph', async () => {
    const app = new App();
    const stack = new Stack(app, 'test');
    const res = new TestResource(stack, 'file:/tmp/x');
    let saved: AppState | undefined;
    ...
    const TestRunnerLayer = RunnerLive.pipe(
      Layer.provide(MockState(
        {
          'file-/tmp/x': { hash: 'old', kind: 'file', metadata: { path: '/tmp/x' } },
          'file:/tmp/x': { hash: 'hash', kind: 'file', metadata: { path: '/tmp/x' } },
        },
        (state) => { saved = state; },
      )),
  ...
  it('warns once when leftover script- or line- hashes cannot match current ids', async () => {
    const app = new App();
    const stack = new Stack(app, 'test');
    new TestResource(stack, 'script:deadbeefdeadbeef', 'hash');
    ...
    const TestRunnerLayer = RunnerLive.pipe(
      Layer.provide(
        MockState({
          'script-echo hello': { hash: 'hash', metadata: {} },
          'line-/tmp/a-export': { hash: 'hash', metadata: {} },
        }),
      ),
    );
    await Effect.runPromise(Effect.provide(program, TestRunnerLayer));

    const leftoverWarnings = warn.mock.calls.filter((call) =>
      String(call[0]).includes('Leftover script-/line- state hashes'),
    );
